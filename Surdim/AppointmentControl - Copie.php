...
function convertDate($dateString) {
    // Extraire la partie de la date sans le fuseau horaire (GMT)
    ...
    if ($timestamp !== false) {
        // Convertir le timestamp dans le format souhaité
...
if (!empty($_GET)){
	...
	if (array_diff($requiredFields, array_keys($_GET)) === []) {
		...
		//$StartDateF = date('Y-m-d H:i:s', strtotime($StartDate)); // Convertir la date en format SQLite
		$EndDate = $_GET['EndDate'];
		$EndDateF = convertDate($EndDate);
		//$EndDateF = date('Y-m-d H:i:s', strtotime($EndDate)); // Convertir la date en format SQLite */
...
/* 		elseif($Methode == "Update"){
			$DataUpdate ="UPDATE surdim_Rent SET Id_List='".$Priority."', User='".ucfirst($User)."', StartDate='".$StartDateF."', EndDate='".$EndDateF."', Info='".$Commentary."', Phone='".$Phone."' WHERE Id = '".$GameID."';";
			$DataSet = PDO_Execute($DataUpdate);
		} */
		elseif($Methode == "Update"){
    // Si c'est un retour anticipé (nouveau paramètre à ajouter dans la requête AJAX)
...
elseif($Methode == "Return"){
    ...
    $DataSet = PDO_Execute($DataUpdate);
		...
		elseif($Methode == "Delete"){
			...
			$returnMessage = "Une erreur est intervenue !";
		}
	}
}
else{
	echo "Les données reçues ne sont pas bonnes!";
}
?>
