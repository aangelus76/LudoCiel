...
onAppointmentAdding: function(e) {
    var newAppointment = e.appointmentData;
    var newStart = new Date(newAppointment.startDate);
    var newEnd = new Date(newAppointment.endDate);
    var newPriority = newAppointment.priority;
    
    // Si c'est un forçage, on laisse passer
    if (newAppointment.forceReservation === true) {
        delete newAppointment.forceReservation; // Nettoyer le flag
        return;
    }
    
    var existingAppointments = scheduler.option("dataSource");
    var conflictingAppointments = [];
    
    for (var i = 0; i < existingAppointments.length; i++) {
        var appointment = existingAppointments[i];
        
        if (appointment.priority !== newPriority) {
            continue;
        }
        
        var existingStart = new Date(appointment.startDate);
        var existingEnd = new Date(appointment.endDate);
    ...
    if (conflictingAppointments.length > 0) {
        ...
        var conflictMessage = "Conflit d'enregistrement ! Cette période chevauche " + 
                             conflictingAppointments.length + 
                             " réservation(s) existante(s) :<br><br>";
        
        for (var j = 0; j < conflictingAppointments.length; j++) {
            var conflict = conflictingAppointments[j];
            var conflictStart = new Date(conflict.startDate);
            var conflictEnd = new Date(conflict.endDate);
            
            var startDay = conflictStart.getDate();
            var startMonth = month[conflictStart.getMonth()];
            var endDay = conflictEnd.getDate();
            var endMonth = month[conflictEnd.getMonth()];
            
            conflictMessage += "• <strong>" + conflict.text + "</strong> : " +
                              "du " + startDay + " " + startMonth + 
                              " au " + endDay + " " + endMonth + "<br>";
        }
        
        // Utiliser DevExpress.ui.dialog au lieu de custom (v21 compatible)
        var dialogOptions = {
            title: "⚠️ Conflit de réservation",
            messageHtml: conflictMessage + "<br><strong>Voulez-vous forcer cette réservation ?</strong>",
            buttons: [{
                text: "Annuler",
                onClick: function() {
                ...
                type: "danger",
                onClick: function() {
                    newAppointment.forceReservation = true;
        ...
        DevExpress.ui.dialog.custom(dialogOptions).show();
    }
},

onAppointmentUpdating: function(e) {
    var updatedAppointment = e.newData;
    var oldAppointment = e.oldData;
    
    var newStart = new Date(updatedAppointment.startDate || oldAppointment.startDate);
    var newEnd = new Date(updatedAppointment.endDate || oldAppointment.endDate);
    var newPriority = updatedAppointment.priority !== undefined ? updatedAppointment.priority : oldAppointment.priority;
    
    if (updatedAppointment.forceReservation === true) {
        delete updatedAppointment.forceReservation;
        return;
    }
    ...
    for (var i = 0; i < existingAppointments.length; i++) {
        ...
        if (appointment.ID === oldAppointment.ID) {
            continue;
        }
    ...
    if (conflictingAppointments.length > 0) {
        ...
        var dialogOptions = {
            title: "⚠️ Conflit de réservation",
            messageHtml: conflictMessage + "<br><strong>Voulez-vous forcer cette modification ?</strong>",
            buttons: [{
                text: "Annuler",
                onClick: function() {
                    scheduler.hideAppointmentPopup();
                ...
                text: "Forcer la modification",
                type: "danger",
                onClick: function() {
                    var forcedAppointment = $.extend({}, oldAppointment, updatedAppointment);
                    forcedAppointment.forceReservation = true;
                    
                    scheduler.deleteAppointment(oldAppointment);
                    scheduler.addAppointment(forcedAppointment);
                    return { buttonText: "Forcer" };
                }
            }]
        };
...
onAppointmentAdded: function(e) {
    showToast('Ajout de : ', e.appointmentData.text, 'success');
	    ...
	    // Nettoyer immédiatement la sélection
    $(".last-cell-selected").removeClass("last-cell-selected");
    $(".open-text").remove();
    $(".dx-scheduler-focused-cell").removeClass("dx-scheduler-focused-cell");
    
    // Utiliser la correspondance pour obtenir l'ID réel
    var priority = e.appointmentData.priority;
    var GameID = priorityToIdMap[priority];
    ...
    // Enregistrer la réservation ET attendre la confirmation avant de recharger
    addAppointment(StartDate, EndDate, User, Info, GameID, Priority, Phone, "Add")
        .done(function(response) {
            // Une fois l'ajout confirmé en base de données, recharger les données
            getAppointments(e.appointmentData.startDate, e.appointmentData.endDate)
                .done(function(appointments) {
                    // Mettre à jour le dataSource avec les données fraîches du serveur
                    scheduler.option("dataSource", appointments);
                    // Mettre à jour les tags après l'ajout
        ...
        .fail(function(error) {
            ...
            showToast('Erreur', 'La réservation n\'a pas pu être enregistrée', 'error');
            // Recharger quand même pour être sûr d'avoir l'état correct
            getAppointments(e.appointmentData.startDate, e.appointmentData.endDate)
                .done(function(appointments) {
                    scheduler.option("dataSource", appointments);
                });
...
/* 		appointmentTooltipTemplate: function(data) {
	...
	//Systeme de visualisation de selection de date
	// Marquer visuellement la dernière cellule sélectionnée
// Marquer visuellement la dernière cellule sélectionnée (UNIQUEMENT sur mouseup)
$(document).on("mouseup", ".dx-scheduler-date-table-cell", function() {
    setTimeout(function() {
        ...
        var $selectedCells = $(".dx-scheduler-focused-cell");
        
        if ($selectedCells.length > 0) {
            // Marquer uniquement la dernière cellule
            var $lastCell = $selectedCells.last();
            $lastCell.addClass("last-cell-selected");
            
            // Ajouter le texte "ouvrir" au centre de la dernière cellule
            var offset = $lastCell.offset();
            var width = $lastCell.outerWidth();
            var height = $lastCell.outerHeight();
...
// Nettoyer quand on clique ailleurs
$(document).on("mousedown", function(e) {
    if (!$(e.target).closest(".dx-scheduler-date-table-cell").length) {
        setTimeout(function() {
            $(".last-cell-selected").removeClass("last-cell-selected");
            $(".open-text").remove();
        }, 50);
...
$(document).on("mousedown", function(e) {
	...
	//Fin Systeme de visualisation de selection de date
	//positionSearchBox();
	//Test scrollTo
...
// Popup Réservation Multiple
var popupMultiple = $("#popupMultiple").dxPopup({
    title: "Réservation multiple",
    width: 1000,
    height: 850,
    contentTemplate: function() {
        const content = $("<div id='popupMultipleContent'>");
        return content;
    },
    onShowing: function() {
        // Charger le formulaire via AJAX
        $("#popupMultipleContent").load("MultipleReservationForm.php");
    },
    onHidden: function() {
        // Nettoyer le contenu au fermeture
        $("#popupMultipleContent").empty();
    },
...
var dateNavigatorContainer = $(".dx-scheduler-navigator");
	...
	var customButton1 = $("<div id='MultipleReserv'>")
	...
	var customButton2 = $("<div id='ViewList'>")
	...
	var customButton4 = $("<div id='Admin'>")
		customButton4.css({'margin-top':'9px'});
		customButton4.css({'margin-right':'10px'});
		customButton4.css({'padding-top':'-5px'});
		customButton4.css({'background-color':'#FFF'});
		customButton4.appendTo(dateNavigatorContainer);
...
customLegend.insertAfter(customButton4);
...
// Bouton Réservation Multiple
	MultipleReservButton = $("#MultipleReserv").dxButton({
		text: "Panier",
		icon: "cart",
		height: "32px",
		stylingMode: "outlined",
